...
export default function VarietyDetailPage() {
  ...
  if (!item) {
    return (
      <div className="text-center py-12">
        <p className="text-[var(--text-secondary)]">综艺不存在</p>
        <Link href="/variety" className="text-[var(--accent)] hover:underline mt-4 inline-block">
          返回综艺列表
        </Link>
      </div>
    );
  ...
  return (
    <div className="flex flex-col gap-6">
      <Link href="/variety" className="text-sm text-[var(--text-secondary)] hover:text-[var(--accent)] flex items-center gap-1">
        ← 返回综艺列表
      </Link>
      ...
      <div className="flex flex-col sm:flex-row gap-6">
        <div className="w-full sm:w-48 md:w-64 flex-shrink-0 mx-auto sm:mx-0 max-w-[256px]">
          <img
            src={item.cover || `https://picsum.photos/seed/v${item.id}/400/600`}
            alt={item.title}
      ...
      {activeTab === 'info' ? (
        <div className="space-y-4">
          <h3 className="font-medium text-[var(--text-primary)]">简介</h3>
        ...
        <div>
          <h3 className="font-medium text-[var(--text-primary)] mb-4">全部期数 ({item.totalEpisode || 0}期)</h3>
      ...
      <div className="mt-6">
        <h3 className="font-medium text-[var(--text-primary)] mb-4">
          {selectedEpisode ? `第${selectedEpisode}期 播放源` : '在线播放'}
        </h3>
        {resourcesLoading || (selectedEpisode && epOnline.length === 0) ? (
          ...
          <p className="text-[var(--text-secondary)] text-sm text-center py-8">
            {selectedEpisode ? '该期暂无资源，请尝试其他期数' : '暂无在线播放资源'}
          </p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3">
