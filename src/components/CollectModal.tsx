      ...
      <div className="relative w-full sm:max-w-md max-h-[70vh] rounded-t-2xl sm:rounded-2xl border flex flex-col"
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b shrink-0" style={{ borderColor: 'var(--border-color)' }}>
          <div className="min-w-0 flex-1">
            <h3 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>收藏到片单</h3>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!isAuthenticated ? (
            <div className="text-center py-8">
              <p className="text-sm mb-3" style={{ color: 'var(--text-muted)' }}>登录后可以收藏影视</p>
              <button onClick={() => { onClose(); router.push(`/login?from=${encodeURIComponent(window.location.pathname)}`); }}
                className="px-4 py-2 rounded-lg text-sm font-medium text-white" style={{ backgroundColor: 'var(--accent)' }}>去登录</button>
            </div>
          ) : (
            <>
                ...
                {defaultLists.map(d => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {d.loaded && <span className="text-[10px]" style={{ color: 'var(--text-muted)' }}>{d.itemCount}部</span>}
                      {isTogglingThis && <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" style={{ color: 'var(--accent)' }} />}
                      {isIn && !isTogglingThis && (
                        <svg className="w-4 h-4" style={{ color: 'var(--accent)' }} viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>
                      )}
                    </button>
                  );
                })}
              ...
              {loading && customLists.length === 0 ? (
                <div className="space-y-2">{[1, 2].map((i) => <div key={i} className="h-14 rounded-lg animate-pulse" style={{ backgroundColor: 'var(--bg-card)' }} />)}</div>
              ) : (
                    ...
                    <div key={list.id}>
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          {list.description && <p className="text-xs truncate mt-0.5" style={{ color: 'var(--text-muted)' }}>{list.description}</p>}
                          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{list.itemCount} 部</p>
                        </div>
                        {isTogglingThis ? (
                          <div className="w-5 h-5 border-2 border-current border-t-transparent rounded-full animate-spin shrink-0" style={{ color: 'var(--accent)' }} />
                        ) : isIn ? (
                          <svg className="w-5 h-5 shrink-0" style={{ color: 'var(--accent)' }} viewBox="0 0 24 24" fill="currentColor"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z" /></svg>
                        ) : (
                          <svg className="w-5 h-5 shrink-0" style={{ color: 'var(--text-muted)' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
                        )}
                      </button>
              ...
              {showCreate ? (
                <div className="mt-2 p-3 rounded-lg border" style={{ borderColor: 'var(--border-color)', backgroundColor: 'var(--bg-card)' }}>
                  ...
                  <div className="flex gap-2 justify-end">
                    <button onClick={() => setShowCreate(false)} className="px-3 py-1.5 rounded-lg text-xs font-medium border" style={{ borderColor: 'var(--border-color)', color: 'var(--text-secondary)' }}>取消</button>
                    <button onClick={handleCreate} disabled={creating || !newName.trim()} className="px-3 py-1.5 rounded-lg text-xs font-medium text-white disabled:opacity-50" style={{ backgroundColor: 'var(--accent)' }}>{creating ? '...' : '创建并收藏'}</button>
                  </div>
                </div>
              ) : (
                <button onClick={() => setShowCreate(true)} className="w-full flex items-center gap-2 p-3 rounded-lg mt-1 transition-colors" style={{ color: 'var(--accent)' }}>
